...
export const fetchBuyerTheCulttCategories = () => async (dispatch: Dispatch<BuyerTheCulttActions>) => {
	const { data } = await $api.get<{ [key: string]: BuyerTheCulttCategory }>(`/buyer_goods`);
...
export const sendBuyerTheCulttProductApplication = (info: { fio: string, phone: string, email: string }, good_id: number) => (dispatch: Dispatch<BuyerTheCulttActions>) => {
	dispatch({
		type: BuyerTheCulttActionTypes.SET_BUYER_THE_CULTT_IS_SEND_FORM_PRODUCT_PAGE,
		payload: true
	})

	$api.post<BuyerTheCulttProduct>(`/send_goods_request/`, { ...info, good_id }).then(() => {
		dispatch({
...
export const sendBuyerTheCulttProductCustomApplication = (info: { fio: string, phone: string, email: string, brand: string, good_link: string, additional_link: string }) => (dispatch: Dispatch<BuyerTheCulttActions>) => {
	dispatch({
