...
const SellChoiceModel: React.FC = () => {
	...
	const { autoDetected: { models, selectedIndex } } = useTypedSelector(({ cabinet_sell }) => cabinet_sell);
	...
	return (
		<div className="sell-block sell-block-choice-model">
			...
			<div className="sell-block-choice-model-btns">
				<button className="btn sell-block-choice-model-btns__btn" onClick={onClickNextStep}>Выбрать</button>

				<button className="btn-regular sell-block-choice-model-btns__btn" onClick={onClickNextStep}>Нет моего варианта</button>
			</div>
		</div>
	);
};
