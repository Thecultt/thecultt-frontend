...
const BuyerTheCulttMainFaq: React.FC = () => {
	...
	return (
		<div className='buyer-thecultt-faq'>
			<h2 className="buyer-thecultt-faq__title">
				Вопросы и ответы
			</h2>
			...
			<div className="buyer-thecultt-faq-items-wrapper">
				{items.map((item, index) => (
					<div className="buyer-thecultt-faq-item" key={`buyer-thecultt-faq-item-${index}`}>
						<h3 className="buyer-thecultt-faq-item__title">
							{item.title}
						</h3>

						<p className="buyer-thecultt-faq-item__description" dangerouslySetInnerHTML={{ __html: item.description }}></p>
					</div>
				))}
