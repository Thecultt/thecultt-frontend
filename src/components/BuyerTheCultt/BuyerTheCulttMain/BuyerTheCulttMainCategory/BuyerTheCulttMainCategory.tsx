...
const BuyerTheCulttMainCategory: React.FC<BuyerTheCulttMainCategoryProps> = ({ title, brands, products, onClickOpenCustomForm }) => {
	...
	return (
		<div className='buyer-thecultt-category'>
			<h2 className="buyer-thecultt-category__title">
				{title}
			</h2>

			<BuyerTheCulttMainCategoryBrands brands={brands} currentFilterBrand={currentFilterBrand} setCurrentFilterBrand={onClickCurrentFilterBrand} />

			<div className="buyer-thecultt-category-products-wrapper">
				{products.filter((product) => currentFilterBrand === "" ? product : product.brand === currentFilterBrand).map((product, index) => (
					<BuyerTheCulttMainCategoryProduct {...product} key={`buyer-thecultt-category-${product.id}-${index}`} />
			...
			<p className="buyer-thecultt-category__subtext">
				*предложение не является публичной офертой.
			</p>

			<p className="buyer-thecultt-category__subtitle">
				К заказу доступны все материалы и размеры из коллекции бренда.
				Если желаемой модели нет в этой подборке, свяжитесь с нами.
			</p>
