...
const FaqBuyers: React.FC = () => {
	const tabs: { title: string, description: string }[] = [
		...
		},
		{
			...
			description: `
			Вы можете оплатить заказ на сайте банковской картой с помощью безопасной системы платежей или воспользоваться беспроцентной рассрочкой или кредитом на сайте, если хотите разбить платеж на части. 
			<br />
			<br />
			При доставке с примеркой по Москве возможна оплата нашему курьеру наличными или банковской картой по ссылке или QR-коду.
			`,
		},
		{
		...
		{
			title: "КАК Я МОГУ ОЗНАКОМИТЬСЯ С АССОРТИМЕНТОМ, ЕСЛИ Я НЕ В МОСКВЕ?",
			description: `
			Мы работаем как интернет-магазин, весь актуальный ассортимент размещен в каталоге сайта thecultt.com. 
			<br />
			<br />
			Ко всем товарам доступно подробное описание состояния и детальные фотографии, включая фото на человеке.
			<br />
			<br />
			Если описания оказалось недостаточно - наши менеджеры всегда готовы ответить на ваши вопросы и выслать дополнительные материалы. Для этого напишите нам в любом удобном вам мессенджере:
			<br />
			<br />
			- Telegram: <a href="https://t.me/thecultt_help">thecultt_help</a>
			<br />
		...
		},
		{
			...
			<br />
			<br />
			1. Приехать в нашу клиентскую зону,  расположенную прямо в нашем офисе. Адрес: г. Москва, ул. Трубная, д. 25, к. 3. Примерка доступна в будние дни с 11:00 до 18:00. Нужна предварительная запись. На примерку у вас будет 30 минут.
			<br />
			<br />
			...
			Мы доставим лоты по удобному вам адресу в пределах МКАД (не более 2 сумок и не более 4 товаров в одном заказе) и вы сможете принять решение о покупке после примерки. Курьер заранее согласует с вами время доставки в промежутке с 11 до 21, с понедельника по воскресенье. На примерку у вас будет 15 минут.
			<br />
			<br />
			...
			<br />
			<br />
			Важно! мы не можем забронировать товары от покупки в интернет-магазине, так как все лоты представлены в единственном экземпляре. Поэтому накануне или в день примерки менеджер может сообщить вам об отсутствии одной или нескольких позиций из вашего списка.
			`,
		},
		{
			title: "НЕ МОГУ НАЙТИ НУЖНЫЙ ТОВАР. ЧТО ДЕЛАТЬ?",
			description: `Если товара, который вы ищете, не оказалось в нашей подборке, просто оставьте заявку в листе ожидания, и мы сообщим вам, как только лот появится в наличии — до публикации в социальных сетях!`,
		}
	]
