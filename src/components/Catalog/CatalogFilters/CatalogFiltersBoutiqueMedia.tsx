...
const CatalogFiltersBoutiqueMedia: React.FC = () => {
	...
	return (
		<div className='catalog-filters-boutique'>
			<button className={`catalog-filters-boutique__btn ${!boutique ? "active" : ""} all`} onClick={() => onClickSetBoutique(false)}>
				Все товары
			</button>
			<button className={`catalog-filters-boutique__btn ${boutique ? "active" : ""} boutique`} onClick={() => onClickSetBoutique(true)}>
				Из бутика
			</button>
		</div>
	)
