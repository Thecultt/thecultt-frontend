...
import { fetchOrderAddressCountrys, fetchOrderAddressCitys, setOrderAddressContry, setOrderAddressCity } from '../../../redux/actions/order'
...
const OrderFormCountry: React.FC = () => {
	...
	const { globalCountrys, globalCitys, address: { country } } = useTypedSelector(({ order }) => order)
				...
				<div className="order-form-block-input" style={{ width: "49%" }}>
					<Field
						...
						onSaveValue={(value: {
							title: string,
							value: string
						}) => dispatch(setOrderAddressCity(value))}
					/>
				</div>
			</div>
		</div>
